...
$(function(){
    ...
    var SingAppView = function(){
        ...
        if (this.pjaxEnabled){
            ...
            $(document).pjax('#sidebar .sidebar-nav a:not([data-toggle=collapse], [data-no-pjax], [href=#])', '#content', {
             ...
             $(document).pjax('.click-action', '#content', {
                fragment: '#content',
                type: 'POST', //this.debug ? 'POST' : 'GET' //GET - for production, POST - for debug.
                timeout: 4000
            });
    ...
    SingAppView.prototype.activeNavigationItem = function(){
         var $newActiveLink = this.$sidebar.find('a[href*="' + this.extractPageName(window.location.href) + '"]').filter(function(){
...
//    SingApp.expandNavigation();
    ...
    initAppFunctions();
    initAppFixes();
    window.SingApp.activeNavigationItem();
    //initDemoFunctions();
});
